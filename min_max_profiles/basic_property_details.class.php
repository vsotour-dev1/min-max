 ...
 *
 * @version Jomres 9.9.5 / rusty version 9.11.1
 *
...
class basic_property_details
{
	...
	//echo "<pre>";print_r ($this->multi_query_result);exit;
            ...
            $this->property_description = $this->multi_query_result[ $this->property_uid ][ 'property_description' ];
			   ...
			   $this->property_days = $this->multi_query_result[ $this->property_uid ][ 'property_days' ]; //rusty 
...
// rusty code start
		...
		foreach ($property_ts as $property_t){
			...
			$p_types_des[]= $property_t['id'];
			}
			
		}

			$show_tour_theme = 0;
		if (in_array($data->ptype_id ,$p_types_des )){ // rusty code check tour properties for show the tour compare theme
					
						$show_tour_theme = 1; 
				}
				
				
			if ($show_tour_theme == 1) { // show country and region for tours properties
				
				$cons = json_decode($data->property_country,true);
				 $countryname = '';
			
				 if (is_array($cons)) {
				foreach ($cons as $c) {
					
					 $countryname .= getSimpleCountry($c).",";	
  ...
  //	print_r ( $countryname); exit;
                ...
                $this->multi_query_result[ $data->propertys_uid ][ 'property_postcode' ] = $data->property_postcode;
				  ...
				  $this->multi_query_result[ $data->propertys_uid ][ 'property_days' ] = $data->property_days;
            

// rusty code check for the regions and country for tours 

	if ($show_tour_theme == 1) { // show country and region for tours properties
				
				$cons = json_decode($data->property_country,true);
			$lat = json_decode($data->lat);
			$long=  json_decode($data->long);
			$reg_ids = json_decode( $data->property_region,true);
			
		//	print_r ($reg_ids);exit;
		if (is_array($reg_ids)) {
				foreach ($cons as $k=>$c) {
					
				// country array
				$this->multi_query_result[ $data->propertys_uid ][ 'property_country' ][$k] = jr_gettext('_JOMRES_CUSTOMTEXT_PROPERTY_COUNTRY', getSimpleCountry($c), $editable, false);
                $this->multi_query_result[ $data->propertys_uid ][ 'property_country_code' ][$k] = $c;
					// lontid and latitude	 array
                $this->multi_query_result[ $data->propertys_uid ][ 'lat' ][$k] = $lat->$k;
                $this->multi_query_result[ $data->propertys_uid ][ 'long' ][$k] = $long->$k;
				
				// regions for tours array
				if (is_numeric($reg_ids[$k])) {
                    $jomres_regions = jomres_singleton_abstract::getInstance('jomres_regions');
				                    
					$this->multi_query_result[ $data->propertys_uid ][ 'property_region' ][$k] = jr_gettext('_JOMRES_CUSTOMTEXT_REGIONS_'.$reg_ids[$k], $jomres_regions->get_region_name($reg_ids[$k]), $editable, false);
                    $this->multi_query_result[ $data->propertys_uid ][ 'property_region_id' ][$k] =$reg_ids[$k];
                } else { // This is a fallback for older properties that used to use region names instead of region ids
                    $this->multi_query_result[ $data->propertys_uid ][ 'property_region' ][$k] = jr_gettext('_JOMRES_CUSTOMTEXT_PROPERTY_REGION', $reg_ids[$k], $editable, false);
                    $this->multi_query_result[ $data->propertys_uid ][ 'property_region_id' ][$k] = 0;
                }
				}
					}else{
