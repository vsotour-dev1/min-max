...
*
*/
function getSimpleCountry_new($selectedCountry = '')
{
	if ($selectedCountry == '') {
		return false;
	}
...
*
*/
function createSimpleCountriesDropdown_new($selectedCountry = '', $input_name = 'guest_country')
{
    $mrConfig = getPropertySpecificSettings();
...
*
*/
function limitCountriesDropdown_new()
{
...
// rusty code fow new funciton for country
...
*
*/
function createCountriesDropdown_new($selectedCountry, $input_name = 'country', $include_onchange = true)
{
	$selectedCountry = strtoupper($selectedCountry);
...
function createCountriesDropdown_new_ajax($selectedCountry, $input_name = 'country', $include_onchange = true)
{
	$selectedCountry = strtoupper($selectedCountry);
...
*
*/
function setupRegions_new($countryCode = 'GB', $currentRegion = 'Pembrokeshire', $firstBlank = false, $input_name = 'region')
{
    $regionArray = array();
	$regionDropdown = '';
    ...
    if (!empty($regionArray)) {
        ...
        foreach ($regionArray as $k => $v) {
		  ...
		  $regionDropdown_1 = "<select id='".$input_name."' name='".$input_name."' class='form-control' ";
	...
	foreach ($regions as $region) {
		if ($currentRegion != '' && $currentRegion == $region->value) {
            $selected = 'selected';
        } else {
            $selected = '';
        }
		$regionDropdown_1 .= "<option value='". $region->value. "' ".$selected." >". $region->text ."</option>";
    }
	
	$regionDropdown_1 .= '</select>';
	
        $regionDropdown = jomresHTML::selectList($regions, $input_name, "class='inputbox'", 'value', 'text', $currentRegion);
		
    }

    return $regionDropdown_1;
}

function setupRegions_new_ajax($countryCode = 'GB', $currentRegion = 'Pembrokeshire', $firstBlank = false, $input_name = 'region')
{
    $regionArray = array();
	$regionDropdown = '';
...
function countryCodesArray_new()
{
    $jomres_countries = jomres_singleton_abstract::getInstance('jomres_countries');
	$jomres_countries->get_all_countries();
...
function old_countryCodesArray_new($translate = true)
{
    $countryCodes = array(
