	...
	//	print_r ($prices)			;
 	...
 	if ( !empty($prices->$show_key ))      $selected_price_row = $prices->$show_key ;// selected price_row from table
		...
		foreach ($activity_guest_values_1 as $price_key=>$value)
		{
		 ...
		 echo '; populateDiv("price_'.$guest_name[0].'_'.$guest_name[1].'","'.output_price($selected_price_row->$guest_name[0]).'")';// rusty code fill out the prices for guest in activity row	
