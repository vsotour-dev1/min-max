...
class j06000show_property_header
{
    public function __construct($componentArgs)
    {
        ...
        $output[ 'STREET' ] = $current_property_details->property_street; // removed for property type tour
...
if (is_array($current_property_details->property_country)){
	$rg = $current_property_details->property_region;
	foreach($current_property_details->property_country as $k=>$coun) {
		
	$output['CR'] .='<span itemprop="addressCountry">'.$coun.'</span>	
		<span itemprop="addressRegion">('.$rg[$k].')</span><br/>';	
	}
		...
		$output['CR'] ='<span itemprop="addressCountry">'.$current_property_details->property_country.'</span>	
		<span itemprop="addressRegion">('.$current_property_details->property_region.')</span><br/>';
}
 ...
 $price_1 = output_price($this->get_property_price_profile($property_uid)); // rusty code changes for min price
 
        $output['PRICE'] = $price_1;
...
//echo $show_tour_theme;exit;
		...
		$plugin_dir = JOMRESCONFIG_ABSOLUTE_PATH.JOMRES_ROOT_DIRECTORY.JRDS.'remote_plugins'.JRDS.'min_max_profiles';
         ...
         $tmpl->setRoot(JOMRES_TEMPLATEPATH_FRONTEND);
		   ...
		   $tmpl->setRoot( $plugin_dir.JRDS."templates".JRDS.'bootstrap3' );
		     $tmpl->readTemplatesFromInput('property_header_tour.html');
			}
	...
	public function get_property_price_profile($property_uid){
		
	 $query = "SELECT main_price FROM #__jomres_jintour_profiles WHERE `property_uid` = '".(int) $property_uid."'";
            $main_prices = doSelectSql($query);
			foreach ($main_prices as $price) {
				if (!empty($price->main_price)){
					
					$m_price[]= $price->main_price; 
				}
