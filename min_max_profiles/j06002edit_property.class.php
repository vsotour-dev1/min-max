...
class j06002edit_property
{
    public function __construct($componentArgs)
    {
        ...
        if ($MiniComponents->template_touch) {
			...
			include_once($ePointFilepath."countries.php"); // rusty
		...
		if (in_array($current_property_details->property_type,$p_types_des )){ // rusty code check tour properties for show the tour compare theme
			...
			if ($show_tour_theme  == 0){
				//regular properties show the country region and lat
...
// rusty to check tours sh
	//echo "<pre>";print_r ($current_property_details);exit;
        ...
        if ($jrConfig[ 'limit_property_country' ] == '0') {
            ...
            $output[ 'AJAX_REGIONDROPDOWN' ] = setupRegions_new($selectedCountry, $current_property_details->property_region, false, 'region');// rusty
        } else {
            $output[ 'COUNTRIESDROPDOWN' ] = getSimpleCountry($jrConfig[ 'limit_property_country_country' ]);
            $output[ 'REGIONDROPDOWN' ] = setupRegions_new($jrConfig[ 'limit_property_country_country' ], $current_property_details->property_region); // rusty

            $output[ 'AJAX_COUNTRIESDROPDOWN' ] = createSimpleCountriesDropdown($jrConfig[ 'limit_property_country_country' ], 'country');
            $output[ 'AJAX_REGIONDROPDOWN' ] = setupRegions_new($jrConfig[ 'limit_property_country_country' ], $current_property_details->property_region, false, 'region'); // rusty
        }
	...
	// $selectedCountry = jomresGetParam($_REQUEST, 'selectedCountry', '');
        ...
        if ($selectedCountry == '') {
            $cons = $current_property_details->property_country_code;
			
			
        }	
			 $output[ 'CON' ]='';
		$regs = $current_property_details->property_region;
...
// rusty to check tours sh
	...
	if (!empty($cons)) {
			if(!is_array($cons)) $cons= array(1=>$cons);
			if(!is_array($regs)) $regs= array(1=>$regs);
			$next_country = COUNT($cons);
			if ($next_country == 0)  $next_country =1 ; 
		
		foreach ($cons as $k=>$selectedCountry) {
      
			
            $output[ 'COUNTRIESDROPDOWN' ] = createCountriesDropdown_new($selectedCountry,'country',false);
            $output[ 'REGIONDROPDOWN' ] = setupRegions_new($selectedCountry, $regs[$k]);
		$output[ 'COUNTRIESDROPDOWNAJAX' ] = createCountriesDropdown_new_ajax($selectedCountry,'country',false);
          
		  $output[ 'REGIONDROPDOWNAJAX' ] = setupRegions_new_ajax($selectedCountry, $regs[$k]);
			

            $output[ 'AJAX_COUNTRIESDROPDOWN' ] = createSimpleCountriesDropdown($selectedCountry, 'country');
            $output[ 'AJAX_REGIONDROPDOWN' ] = setupRegions_new($selectedCountry, $regs[$k], false, 'region');
        ...
        //TODO: is this still needed?
        if ($output[ 'LAT' ] == '') {
            $url = 'https://maps-api-ssl.google.com/maps/api/geocode/json?address='.urlencode($current_property_details->property_name).','.urlencode($regs[$k]).','.urlencode($selectedCountry);
...
<div class="clear" ></div>	
<div class="row_'.$k.'" id="row_'.$k.'">
<div style="clear:both;" />
<div class="col-md-2 table_row_1 first_row">';

if ($k == 1){
	 $output[ 'CON' ]	.= '		<span  class="plus_img col-md-1" id="button_add" data-row="'.$next_country.'"></span>';
}else{
	$output[ 'CON' ]	.= '<span onclick=\'myFunction("'.$k.'")\'   class="minus_img col-md-1" id="button_remove" data-row="'.$k.'"></span>';
		...
		if ($k == 1){	
		$output[ 'CON' ]	.= '</div>
	 <div class="col-md-2 table_row_1"> <select id="country" name="country" class="form-control">'.$output[ 'COUNTRIESDROPDOWNAJAX' ].'</select>	</div>
		<div class="col-md-2 table_row_1" id="property_region"> <select id="region" name="region" class="form-control">'.$output[ 'REGIONDROPDOWNAJAX'].' </select>	</div>';

$output[ 'CON' ]	.= '	<div class="col-md-2 table_row_1"> 	<input id="lat" name="lat" type="text" size="20" value="'.  $output[ 'LAT' ].'" class="form-control" placeholder="00.0000"/> 	</div>
		<div class="col-md-2 table_row_1"> 	<input id="lng" name="long" type="text" size="20" value="'. $output[ 'LONG' ] .'" class="form-control" placeholder="00.0000"/>	</div>';
	

		}else{
				$output[ 'CON' ]	.= '</div>
	 <div class="col-md-2 table_row_1"> <select id="country_'.$k.'" name="country_'.$k.'" class="form-control">'.$output[ 'COUNTRIESDROPDOWNAJAX' ].'</select>	</div>
		<div class="col-md-2 table_row_1" id="property_region_'.$k.'"> <select id="region_'.$k.'" name="region_'.$k.'" class="form-control">'.$output[ 'REGIONDROPDOWNAJAX'].' </select>	</div>';
	$output[ 'CON' ]	.= '	<div class="col-md-2 table_row_1"> 	<input id="lat_'.$k.'" name="lat_'.$k.'" type="text" size="20" value="'.  $output[ 'LAT' ].'" class="form-control" placeholder="00.0000"/> 	</div>
		<div class="col-md-2 table_row_1"> 	<input id="lng_'.$k.'" name="long_'.$k.'" type="text" size="20" value="'. $output[ 'LONG' ] .'" class="form-control" placeholder="00.0000"/>	</div>';

		
		
	}

	$output[ 'CON' ]	.= '	<div class="clear" ></div>	
		</div>	
		';
		...
		$output[ 'CON' ] =$this->country($current_property_details);
		  $output[ 'COUNTRIESDROPDOWN' ] = createCountriesDropdown_new('TH','country',false);
            $output[ 'REGIONDROPDOWN' ] = setupRegions_new('TH', '');
		$output[ 'COUNTRIESDROPDOWNAJAX' ] = createCountriesDropdown_new_ajax('TH','country',false);
          
		  $output[ 'REGIONDROPDOWNAJAX' ] = setupRegions_new_ajax('TH', '');
  ...
  $output[ 'PROPERTY_DAYFORM' ] = "";
        ...
        if ($jrConfig[ 'allowHTMLeditor' ] == '1') {
          ...
          //  echo  $output[ 'PROPERTY_DESCRIPTION' ];exit;
			...
			$output[ 'PROPERTY_DAY_ACT' ] = editorAreaText('activity_desc', $current_property_details->property_days, 'activity_desc', $width, $height='100', '10', 2);
           
      
		$days = json_decode($current_property_details->property_days);
		//echo "<pre>asdfas";print_r ($days);		print_r ($current_property_details->property_days );exit;
		$div_html = '';
	...
	foreach($days as $key=>$d) {
		...
		foreach ($d as $k=>$v){
			$day_idis[]= $k;
		$div_html .= "<div style='clear:both'></div><div id='row_day_".$k."' class='row_1' >";
		$div_html .= "<div id='button_edit_days".$k."' data-row='".$k."' class='btn btn-md btn-primary add_days' >
		<i class='fa fa-check'></i>".$v->day_title;		
 $div_html  .= "<input type='hidden' name='tour_days[".$k."]' id='tour_days_".$k."' value='".json_encode($d, JSON_UNESCAPED_UNICODE)."' /> </div>";
			
		$div_html .= "<div class='inp_move'><div class='move_del'><span class='minus_img' onclick='remove(this)'>remove</span></div></div></div>";
		
		}
	
	}
	
	$div_html  .= "<div style='clear:both'></div><input type='hidden' name='day_idis' id='day_idis' value='".json_encode($day_idis)."' /> ";

	   $output ['PROPERTY_DAYS'] =  $div_html ;
			...
			$output['SIMPLEMDE_JAVASCRIPT'] = '
				...
				jomresJquery(document).ready(function () {
					...
					var simplemde = new SimpleMDE({ element: document.getElementById("activity_desc") ,toolbar: buttons, });
...
$ePointFilepath = get_showtime('ePointFilepath');
	if (file_exists($ePointFilepath.'language'.JRDS.get_showtime('lang').'.php')) {
        ...
        $output[ 'HPROPDAYS' ] = jr_gettext('_JOMRES_COM_MR_VRCT_PROPERTY_DAYS', '_JOMRES_COM_MR_VRCT_PROPERTY_DAYS');  // rusty code
...
$output[ 'LONG' ]= 100.538364;
$output[ 'LAT' ]= 13.764770;
		...
		$tmpl->setRoot( $plugin_dir.JRDS."templates".JRDS.'bootstrap3' );
			...
			if($show_tour_theme==1){
		   
		     $tmpl->readTemplatesFromInput('edit_property_tour.html');
			}else{
	...
	public function country($current_property_details){
		...
		$long = $current_property_details->long;
		 ...
		 $output[ 'COUNTRIESDROPDOWNAJAX' ] = createCountriesDropdown_new_ajax($selectedCountry,'country',false);
            $output[ 'REGIONDROPDOWNAJAX' ] = setupRegions_new_ajax($selectedCountry, $regs[1]);
        ...
        //TODO: is this still needed?
        if ($output[ 'LAT' ] == '') {
            $url = 'https://maps-api-ssl.google.com/maps/api/geocode/json?address='.urlencode($current_property_details->property_name).','.urlencode($regs[1]).','.urlencode($selectedCountry);
...
<div class="col-md-2 table_row_1 first_row">';
	 ...
	 $output_row	.= '		<span  class="plus_img col-md-1" id="button_add" data-row="1"></span>';
 ...
 $output_row	.= '	<div class="col-md-2 table_row_1"> 	<input id="lat" name="lat" type="text" size="20" value="'.  $output[ 'LAT' ].'" class="form-control" placeholder="00.0000"/> 	</div>
		<div class="col-md-2 table_row_1"> 	<input id="lng" name="long" type="text" size="20" value="'. $output[ 'LONG' ] .'" class="form-control" placeholder="00.0000"/>	</div>';
	

		

 $output_row	.= '	<div class="clear" ></div>	
		</div>	
		';
