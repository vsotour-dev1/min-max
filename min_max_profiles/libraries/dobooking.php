...
if (!empty($result)) {
    if ($selectedProperty > 0) {
        ...
        dobooking($selectedProperty, $thisdate, $remus,$epoint); // rusty code
 ...
 */
function dobooking($selectedProperty, $thisdate, $remus,$epoint) // rusty code
{
    $thisJRUser = jomres_singleton_abstract::getInstance('jr_user');
    ...
    $guestTypes = makeCustomerTypes_new($selectedProperty,$bkg); // rusty code
    ...
    if ((!empty($extra_details) || !empty($third_party_extras)) && $mrConfig[ 'showExtras' ] == '1') {
        $output[ 'EXTRAS_INFO' ] = '<img border="0" style="vertical-align:top;" src="'.JOMRES_IMAGES_RELPATH.'info.png" />'; // 9.9.19
 ...
 /* removed in 9.9.19 rusty comments   if (get_showtime('mobile_browser')) {
...
if (!get_showtime('include_room_booking_functionality')) {
	$tmpl->setRoot($epoint."templates/bootstrap3/");		


$tmpl->readTemplatesFromInput('dobooking_norooms_tour.html'); // rusty code
} else {
if (($mrConfig[ 'singleRoomProperty' ] == '1')) {
...
//rusty code function
     function makeCustomerTypes_new($selectedProperty,$bkg)
    {
           $mrConfig = getPropertySpecificSettings($selectedProperty);
        $cust = array();
        ...
        foreach ($exList as $ct) {
            ...
            $customerTypes[ 'TEXT' ] = $bkg->sanitiseOutput(jr_gettext('_JOMRES_CUSTOMTEXT_GUESTTYPE'.$ct->id, htmlspecialchars(trim(stripslashes($ct->type)), ENT_QUOTES)));
            $current = $bkg->getGuestVariantDetails($ct->id);
            ...
            if ($current != false) {
               ...
               $bkg->setGuestVariantDetails($ct->id, $defNo);
                $bkg->total_in_party = $bkg->total_in_party + $defNo;
            } else {
                if (empty($cust)) {
            ...
            $customerTypes[ 'DROPDOWN' ] = jomresHTML::integerSelectList($default_start, $ct->maximum, 1, 'guesttype'.$ct->id, 'size="1" onchange="getResponse_guesttype_new(\''.$ct->id.'\',this.value);"', $defNo, '0', $use_bootstrap_radios = false);
            $customerTypes[ 'FIELDNAME' ] = 'guesttype'.$ct->id;
			// rusty
			 $customerTypes[ 'HIDDEN_INPUT' ] = '<input type="hidden" class="guest_values" name="guest_values[]" value='.$ct->id.' />';
            $cust[ ] = $customerTypes;
        }
